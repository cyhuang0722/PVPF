...
def save_forecast_band_plot(df: pd.DataFrame, out_path: str | Path, title: str) -> None:
    ...
    ax.plot(df["ts_target"], df["pred_q50_w"], label="Pred q50", color="tab:blue", linewidth=1.4)
    ax.fill_between(df["ts_target"], df["pred_q10_w"], df["pred_q90_w"], alpha=0.25, color="tab:blue", label="q10-q90")
    ax.set_title(title)
