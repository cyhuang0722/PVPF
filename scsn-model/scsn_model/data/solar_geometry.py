...
class Calibration:
    ...
    def inferred_reference_size(self) -> tuple[int, int]:
        ...
        # Fallback: infer from fitted optical center when the raw image size is not stored.
        # The current calibration was fit on the original fisheye frames, whose center is
        # close to the image midpoint. This gives a much better scaling base than assuming
        # the already-resized training image is the calibration reference.
        ref_w = int(round(self.cx * 2.0))
        ref_h = int(round(self.cy * 2.0))
        return ref_w, ref_h
    ...
    def rescale(self, dst_w: int, dst_h: int) -> "Calibration":
        ...
        return Calibration(
            cx=self.cx * sx,
            cy=self.cy * sy,
            f_px_per_rad=self.f_px_per_rad * s,
            lat=self.lat,
            lon=self.lon,
...
def project_sun_to_image(
    ...
    r = calib.f_px_per_rad * theta_rad
    direction = 1.0 if azimuth_clockwise else -1.0
    ang = np.deg2rad(direction * azimuth_deg + azimuth_offset_deg)

    x = calib.cx + r * np.sin(ang) + float(image_offset_x_px)
    y = calib.cy - r * np.cos(ang) + float(image_offset_y_px)
    x = np.clip(x, -0.5, image_width - 0.5)
    y = np.clip(y, -0.5, image_height - 0.5)
    return x, y
...
def build_solar_feature_vector(
    ...
    return np.array(
        [x_norm, y_norm, np.sin(az_rad), np.cos(az_rad), zen_norm],
        dtype=np.float32,
    )
...
def compute_clear_sky_power(
    ...
    if ts.tz is None:
        ...
        ts = ts.tz_convert(calib.timezone)
    site = location.Location(calib.lat, calib.lon, tz=calib.timezone)
    clearsky = site.get_clearsky(ts, model="ineichen")
    ghi = clearsky["ghi"].fillna(0.0).to_numpy(dtype=np.float32)
    if np.nanmax(ghi) <= 0:
        power = np.full_like(ghi, floor_w, dtype=np.float32)
    else:
        power = peak_power_w * (ghi / np.nanmax(ghi))
        power = np.maximum(power, floor_w)
    return pd.Series(power.astype(np.float32), index=ts)
