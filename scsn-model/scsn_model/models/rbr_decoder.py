...
class FutureRBRDistributionDecoder(nn.Module):
    def __init__(self, spatial_dim: int, latent_dim: int, hidden_dim: int, feature_hw: int) -> None:
        ...
        self.rbr_variance_head = nn.Conv2d(trunk_channels, 1, kernel_size=3, padding=1)
    ...
    def forward(self, future_z: torch.Tensor, hidden_seq: torch.Tensor, spatial_feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        rbr_variance = 0.25 * torch.sigmoid(self.rbr_variance_head(trunk)).view(batch, steps, 1, self.feature_hw, self.feature_hw)
        rbr_logvar = torch.log(rbr_variance.clamp_min(1e-6))
        return {
