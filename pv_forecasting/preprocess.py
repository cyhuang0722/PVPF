"""
Forecasting 数据预处理：60 分钟天空图 + 过去 4 个 15min PV + 预测 5 个 horizon。
样本以 15 分钟为锚点 t：天空图 t-59..t，过去 PV [t-60,t-45,t-30,t-15]，目标 [t,t+15,t+30,t+45,t+60]。
...
def build_forecast_windows(
    ...
    """
    对每个有足够历史的 15min 时刻 t，构建：60 张图路径、4 个过去 PV、5 个目标 PV。
    ...
    ts_power = df_pv["ts_power"].values
    power = df_pv["power"].to_numpy(dtype=np.float32)
    n_pv = len(df_pv)
    ...
    for i in range(past_pv_len, n_pv - horizon + 1):
        t = pd.Timestamp(ts_power[i])
        if t.tz is None:
            t = t.tz_localize(tz)
        else:
            t = t.tz_convert(tz)
        ...
        past_pv = power[i - past_pv_len : i].tolist()
        targets = power[i : i + horizon].tolist()
        samples.append({
            "ts_pred": t,
            "img_paths": paths,
            "past_pv": past_pv,
    ...
    # 日志
    print(f"  [match] samples built: {len(out)}, filtered (missing img): {filtered_missing}")
    if delta_secs:
        delta_arr = np.array(delta_secs)
        print(f"  [match] delta_sec: min={delta_arr.min():.2f}s, max={delta_arr.max():.2f}s, "
...
def pack_forecast_to_npz(
    ...
    每片: sky (B,60,3,H,W), past_pv (B,4,1), targets (B,5), ts_pred (B,) object。
    """
    import ast
    ...
    for start in range(0, n, batch_size):
        ...
        targets = np.zeros((B, 5), dtype=np.float32)
        ts_pred = []
...
def main():
    ...
    df_pv = df_pv.dropna(subset=["power"]).query("power > 0").reset_index(drop=True)

    if len(df_pv) < PAST_PV_LEN + HORIZON:
        raise RuntimeError("Not enough PV points")
    print(f"  pv rows: {len(df_pv)}")

    print("[3] Build forecast windows (60 img + 4 past PV + 5 targets)...")
