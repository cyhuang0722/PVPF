...
def build_forecast_windows(
    ...
    if img_len <= 0:
        raise ValueError(f"img_len must be positive, got {img_len}")
    if img_stride_min <= 0:
    ...
    for t in pv_index:
        ...
        ts_list = [
            t - pd.Timedelta(minutes=j * img_stride_min)
            for j in range(img_len - 1, -1, -1)
        ]
        paths = []
        used: set[int] = set()
...
def pack_forecast_to_npz(
    ...
    每片: sky (B,T,3,H,W), past_pv (B,4,1), targets (B,4), ts_pred (B,) object。
    """
    import ast
    ...
    for start in range(0, n, batch_size):
        ...
        B = end - start
        sample0 = df.iloc[start]
        img_len = len(sample0["img_paths"])
        past_pv_len = len(sample0["past_pv"])
        horizon = len(sample0["targets"])
        sky = np.zeros((B, img_len, 3, img_size[0], img_size[1]), dtype=np.float32)
        past_pv = np.zeros((B, past_pv_len, 1), dtype=np.float32)
        targets = np.zeros((B, horizon), dtype=np.float32)
        ts_pred = []
...
def main():
    ...
    parser.add_argument("--cam-dir", default="data/cam_dir", help="天空图目录")
    parser.add_argument("--pv-csv", default="data/power/power-LSK_N.csv", help="PV 15min 数据 CSV")
    parser.add_argument("--out-dir", default="derived", help="输出目录，相对 pv_forecasting/")
    parser.add_argument("--pack-dir", default=None, help="打包输出目录，默认 <out-dir>/packed_forecast")
    parser.add_argument("--pack", action="store_true", help="打包为 .npz")
    ...
    if args.pack:
        pack_dir = (Path(__file__).resolve().parent / args.pack_dir) if args.pack_dir else (out_dir / "packed_forecast")
