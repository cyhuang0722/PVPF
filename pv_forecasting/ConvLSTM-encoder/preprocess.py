...
DEFAULT_OUT_CSV = BASE_DIR / "derived" / "samples_t_plus_15.csv"
...
def build_samples(
    ...
    for target_ts, target_value in zip(power_df["date"], power_df["value"]):
        anchor_ts = target_ts - pd.Timedelta(minutes=15)
        input_ts_list = [anchor_ts - pd.Timedelta(minutes=o) for o in input_offsets_min]
        img_paths: list[str] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build ConvLSTM t+15 training samples")
